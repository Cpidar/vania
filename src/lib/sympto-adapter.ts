...
import { CycleState } from 'src/store/cycle/models';
import { cycleState, getCycleModule } from '../store/cycle/state'
import { CycleInfo, FertalityStatus, OvuPhaseModel, StatusModel } from './cycle.models'
import getFertilityStatus from './sympto'

export function getFertilityStatusForDay(state: CycleState, dateString: string) {
  const status = getCycleStatusForDay(state, dateString)
  const prognancyMode = true
  if (!status) {
    return {
      status: prognancyMode ?  labels.infertile : labels.fertile,
      phase: null
    }
  }
...
export function getCycleStatusForDay(state: CycleState, dateString: string, opts = {} as any) {
  const options = opts || { excludeEarlierCycles: false }
  const {
    getCycleForDay,
    getCyclesBefore,
    getPreviousCycle
  } = getCycleModule(cycleState)
...
function formatStatus(phaseNameForDay: string, dateString: string, status: StatusModel): FertalityStatus {
  const mapping: { [key: string]: any } = {
    ...
    periOvulatory: (dateString: string, status: StatusModel) => {
      ...
      const phaseStart = peri && peri.start
      const phaseEnd = peri && peri.end
      let s: string
      if (phaseStart && phaseStart.date === dateString) {
        s = 'fertile-start'
      } else if (phaseEnd && phaseEnd.date === dateString) {
        s = 'fertile-end'
    ...
    postOvulatory: (dateString: string, status: StatusModel) => ({
      status: labels.infertile,
      phase: 3,
      statusText: status.temperatureShift ? labels.postOvuText(status.temperatureShift.rule) : labels.preOvuText
    }),
    preOvulatoryStd: () => {
      return {
        status: labels.infertile,
